...
int ABNeuron::derivative(realtype t, N_Vector *y, N_Vector *ydot, void *user_data) {
    ...
    v = yi[idx]; h_m=yi[idx+1]; a_m = yi[idx+2]; a_h = yi[idx+3]; kd_m = yi[idx+4];
    cat_m = yi[idx+5];cat_h = yi[idx+6]; cas_m = yi[idx+7];
    cai = yi[idx+8]; kca_m = yi[idx+9]; nap_m = yi[idx+10]; nap_h = yi[idx+11];
    v_a = yi[idx+12]; k_m = yi[idx+13]; na_m=yi[idx+14];
    na_h=yi[idx+15]; proc_m=yi[idx+16];
    
        /*compute calcium reversal potential using Nernst equation  (RT/zF)*(ln(cao/cai))*/
        //2.303*(RT/zF)*(cao/cai)*1000
        // T = 283, R = 8.135, F = 96840
        /* nernst equation is expressed in terms of log base 10. To
         * convert natural log to log base 10 use conversion factor
         * 2.303 */
    ...
    kca_minf = (cai/(cai+30))/((1+exp(-(v+51)/4)));
    ...
    Il = 0.045*(v+50);
    Ih=0.054*h_m*(v+20);
    Ia =200*pow(a_m, 4)*a_h*(v+80);
    Ikd = 1890*pow(kd_m, 4)*(v+80);
    ICaT = 55.2*pow(cat_m, 3)*cat_h*(v-eca);
    ICaS = 9*pow(cas_m, 3)*(v-eca);
    IKCa =6000*pow(kca_m, 4)*(v+80);
    INaP = 2.7*pow(nap_m, 3)*nap_h*(v-50);
    IProc = 570*proc_m*(v-0);
    

        /*axon currents */
    Ik_axon=52.5*pow(k_m, 4)*(v_a+80);
    Il_axon=0.0018*(v_a+60);
    Ina_axon=300*pow(na_m, 3)*na_h*(v_a-50);
...
void ABNeuron::currents(realtype  t, N_Vector y) {
     realtype *yi;
    yi = NV_DATA_S(y);
