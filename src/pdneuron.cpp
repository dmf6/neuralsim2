...
int PDNeuron::derivative(realtype t, N_Vector *y, N_Vector *ydot, void *user_data) {
    ...
    cat_htau = 300-350/(1+exp(-(v+50)/16.9));

    cas_minf = 1/(1+exp(-(v+22)/8.5));
    cas_mtau =  16-13.1/(1+exp(-(v+25.1)/26.4));

    kca_minf = (cai/(cai+30))*(1/(1+exp(-(v+51)/8)));
    ...
    Il = 0.105*(v-EL);
    Ih=0.219*h_m*(v-EH);
    Ia =39.42*pow(a_m, 4)*a_h*(v-EK);
    Ikd = 1576.8*pow(kd_m, 4)*(v-EK);
    ICaT = 22.5*pow(cat_m, 3)*cat_h*(v-eca);
    ICaS = 60*pow(cas_m, 3)*(v-eca);
    IKCa =251.85*pow(kca_m, 4)*(v-EK);
    INaP = 4.38*pow(nap_m, 3)*nap_h*(v-ENA);
    IProc = 0*proc_m*(v-EPROC);
    
        /*axon currents */
    Ik_axon=150*pow(k_m, 4)*(v_a-EK);
    Il_axon=0.00018*(v_a-EL_AXON);
    Ina_axon=1110*pow(na_m, 3)*na_h*(v_a-ENA);
...
void PDNeuron::currents(realtype  t, N_Vector y, N_Vector ydot, ostream& out) {
    realtype *yi;
    yi = NV_DATA_S(y);
